...
void lcd_show_pic(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint8_t *color);          //显示图片，测试图片数据在pic.h中 将其赋给color即可
void lcd_show_inverted_pic(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint8_t *color);//显示倒立的图像
void lcd_show_inverted_pic_cut(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint8_t *color);//显示倒立的图像，去除白色部分
void lcd_show_inverted_pic_full( uint16_t height, uint16_t  width, uint8_t *color);//显示倒立的图像，全屏显示
...
void lcd_show_points(uint16_t x, uint16_t y, uint8_t* chr,uint16_t size,  uint8_t mode, uint16_t color);//显示字库中的点阵
#endif
