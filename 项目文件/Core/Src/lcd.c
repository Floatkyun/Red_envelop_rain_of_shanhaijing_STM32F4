...
uint32_t g_back_color  =  LGRAY;    /* 背景色 */
...
void lcd_show_inverted_pic(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint8_t *color)
{
	...
	cursory=sy+height-1;
		...
		LCD->LCD_RAM =(((uint16_t)color[i+1])<<8)|((uint16_t)color[i]);
...
void lcd_show_inverted_pic_cut(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint8_t *color)
{
	...
	cursory=sy+height-1;
				...
				
		{
					...
					if(((((uint16_t)color[i+1])<<8)|((uint16_t)color[i]))!=0xffff)
					{
...
void lcd_show_inverted_pic_full( uint16_t height, uint16_t  width, uint8_t *color)
{
		uint8_t picH,picL;
...
void lcd_show_points(uint16_t x, uint16_t y, uint8_t* chr,uint16_t size,uint8_t mode, uint16_t color)
{
    uint8_t temp, t1, t;
    uint16_t y0 = y;
		uint16_t x0 = x;
    int csize = size*size/8;
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)                  /* 一个字节8个点 */
        {
            ...
            if ((x - x0) == size)                   /* 显示完一列了? */
            {
                ...
                if (y >= y0+size-size/12||y >=lcddev.height)
                {
                    return;       /* x坐标超区域了 */
                }
...
//(mode == 0)                     /* 无效点,不显示 */
int lcd_show_font_str(uint16_t x,uint16_t y,char *str,uint16_t size,uint16_t color,int mode)//从sd卡显示中文、英文、数字混合字符串
{
	uint8_t len=strlen((char*)str);
	uint8_t idex=0;
	for(idex=0;idex<len;)
	{
		if((uint8_t)str[idex]<0x80)
		{
			lcd_show_char(x, y,str[idex] , size,  mode, color);
			x+=size/2;
			idex++;
		}
		else
		{
		show_chinese_char((uint8_t*)&str[idex] , size,  x, y);
			x+=size;
			idex+=2;
		}
	}
